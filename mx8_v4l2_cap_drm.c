...
static void print_help(const char *name)
{
	v4l2_info("CSI Video4Linux capture Device Test\n"
		   ...
		   " -fmt <format> support XR24, AR24, RGBP, RGB3, BGR3, YUV4, YM24, YUYV, GREY and NV12, only XR24, AR24 and RGBP support playback\n"
...
#else
			fd = open(video_ch[i].save_file_name, O_RDWR | O_CREAT | O_TRUNC, 0775);
#endif
			if (fd < 0) {
...
{
	struct v4l2_dbg_chip_info chipident;
	struct v4l2_format fmt;
	...
	ret = ioctl(fd, VIDIOC_DBG_G_CHIP_INFO, &chipident);
	if (ret < 0)
		v4l2_err("get chip info fail\n");
	else
		v4l2_dbg("Get chip info: %s\n", chipident.match.name);
	...
	ret = ioctl(fd, VIDIOC_S_CTRL, &ctrl);
	if (ret < 0) {
		v4l2_err("channel[%d] VIDIOC_S_CTRL set brightness failed\n", ch_id);
		return ret;
	}
